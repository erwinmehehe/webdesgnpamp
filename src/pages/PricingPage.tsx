    ...
    <>
      ...
      <Section divider>
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, margin: "-64px" }} transition={{ staggerChildren: 0.12, delayChildren: 0.08 }} className="mt-14 grid items-stretch gap-6 lg:grid-cols-3">
          {carePlans.map((plan) => (
            ...
            >
              {plan.featured && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-gradient-to-r from-gold-300 to-gold-500 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-ink-950">Most chosen</span>
              )}
              ...
              <div className="mt-6 flex items-baseline gap-1.5">
                <span className="font-display text-[2.25rem] font-bold leading-none tracking-tight text-white">{plan.price}</span>
        ...
        <Reveal delay={0.15}>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-3xl border border-white/[0.07] bg-white/[0.03] px-7 py-6">
              <p className="font-display text-base font-semibold text-white">Hosting and domain</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-400">We recommend and set up reliable hosting in your name. Typical business hosting runs a few thousand pesos per year and is paid directly by you.</p>
            </div>
            <div className="rounded-3xl border border-white/[0.07] bg-white/[0.03] px-7 py-6">
              <p className="font-display text-base font-semibold text-white">No plan? Still supported</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-400">Past clients can always reach us for help on an hourly or per-task basis — a plan simply makes it cheaper and proactive.</p>
            </div>
          </div>
        </Reveal>
      </Section>
      ...
      <Section ambient divider>
        <div className="grid gap-12 lg:grid-cols-[1fr_1.3fr] lg:gap-16">
          <div>
            <Reveal><Eyebrow>Optional add-ons</Eyebrow></Reveal>
            <Reveal delay={0.08}>
              <h2 className="mt-5 font-display text-3xl font-bold leading-[1.12] tracking-tight text-white sm:text-4xl">Extra work, priced <span className="text-gold-gradient">separately.</span></h2>
            </Reveal>
            <Reveal delay={0.14}>
              <p className="mt-5 text-base leading-relaxed text-slate-400">Nothing here is required to launch a good website. They&rsquo;re useful when you want us to handle content, photography, campaigns or additional functionality.</p>
            </Reveal>
            <Reveal delay={0.2}>
              <div className="mt-7">
                <CheckList items={["Fixed price agreed before each piece of work", "No retainer required for add-on projects", "Can be phased across months if budget is tight"]} />
              </div>
            </Reveal>
          </div>
      ...
      <Section divider>
        <SectionIntro eyebrow="Pricing questions" title="What owners ask about cost and terms" description="Straight answers about budgets, timelines, hosting and what happens if scope changes." />
        <div className="mx-auto mt-12 max-w-3xl">
          <FAQAccordion items={[generalFaqs[0], generalFaqs[1], generalFaqs[4], generalFaqs[6]]} />
        </div>
      </Section>
